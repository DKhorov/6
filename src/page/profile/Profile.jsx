...
const EltonP = ({profile}) => {
  if (profile.accountType === 'admin') return <> 
    <center><img src={elton} style={{padding:'5px',height:'500px'}} alt="Elton" /></center> 
    <Typography sx={{
      fontSize: '15px',
...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const isCurrentUser = currentUserId === userData._id || currentUserId === userData.id;
        const isSubscribed = !isCurrentUser && userData.followers?.some(follower => 
  ...
  if (state.error || !state.user) return <div>Ошибка: {state.error || 'User not found'}</div>;
    ...
    >
      ...
      >
        ...
        <Box sx={{ flex: 1, textAlign: 'center', zIndex: 1 }}>
          ...
          <Typography variant="h6" sx={{ fontWeight: 700, fontFamily: 'Inter, sans-serif',color:"rgba(200, 200, 200, 1)" }}>{subscriptionsCount}</Typography>
        </Box>
      </Box>
      ...
      {!isCurrentUser && (
        <Button
          sx={{ 
            width: '100%',
            backgroundColor: state.isSubscribed ? 'rgba(194, 194, 194, 1)' : 'primary.main',
            color: 'black',
            borderRadius: '20px',
            marginTop: '10px',
            fontWeight: 'Bold',
      ...
      <Box sx={{ px: isMobile ? 2 : 0, mt: 2 }}>
        ...
        {/* Баланс (только для текущего пользователя) */}
        {isCurrentUser && (
          <>
