...
const PostHeaderAcc = ({ 
  ...
  // Защита от undefined и установка значений по умолчанию
  const safePost = {
    ...post,
    user: post.user || {},
    likes: {
  ...
  const handleReaction = async (type) => {
    ...
    try {
      ...
      // Обновляем пост, если сервер вернул обновленные данные
    ...
    <Box sx={{ 
      ...
      <Box sx={{ display: 'flex', alignItems: 'center' }}>
        ...
        <Box sx={{ 
          ...
          <Typography variant="caption" sx={{ mr: 1, minWidth: '10px', fontWeight: 'Bold', color: 'green', ml: 0.5 }}>
            {localLikes}
          </Typography>
          ...
          <Typography variant="caption" sx={{ minWidth: '10px', color: 'red', ml: 0.5, fontWeight: 'Bold' }}>
            {localDislikes}
          </Typography>
        </Box>
          ...
          <>
            ...
            >
              <MenuItem onClick={handleDelete}>
                <ListItemIcon><FiTrash2 size={16} /></ListItemIcon>
                <ListItemText>Удалить пост</ListItemText>
              </MenuItem>
            </Menu>
          </>
        )}
